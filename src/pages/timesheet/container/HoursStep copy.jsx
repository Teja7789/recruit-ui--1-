...
function HoursStep({ onNext, onPrevious, selectedDates, totalDays, selectedTimings, timingValues }) {
  const [weekData, setWeekData] = useState([]);
  const toast = useRef(null);

  //from Timesheet totals hours//
...
// console.log(timingValues);
  ...
  useEffect(() => {
    generateWeekData();
  }, [selectedDates.startDate, selectedDates.endDate, selectedTimings]);
  ...
  const renderHeaders = () => {
    const timingLabels = ['Days', ...selectedTimings, 'Overall Total'];
  ...
  return (
    <div>
      ...
      <div className="p-mt-4 form-btns">
        <Button className='mr-2 company-secondary-btn' label='Previous' onClick={() => onPrevious("timesheet")} />
        <Button label="Next" className='company-primary-btn' onClick={handleNextClick} type="button" />
      </div>
